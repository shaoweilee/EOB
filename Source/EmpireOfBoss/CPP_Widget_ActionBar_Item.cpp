...
void UCPP_Widget_ActionBar_Item::OnSlotClicked()
{
	...
	AEmpireOfBossPlayerController* EOB_PC = Cast<AEmpireOfBossPlayerController>(GetOwningPlayer());
	UE_LOG(LogTemp, Warning, TEXT("---按了下技能---"));
	if (EOB_PC && EOB_PC->UABC && AssignedSkillTag.IsValid())
	{
		// 🌟 暗黑流标准闭环：点击格子，直接通过标签让 GAS 释放对应技能！
		FGameplayTagContainer AbilityContainer(AssignedSkillTag);
		EOB_PC->UABC->TryActivateAbilitiesByTag(AbilityContainer);
